          ...
          <motion.div key="playing" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="relative flex-1 flex flex-col h-full">
            <div className="absolute top-0 left-0 right-0 z-20 px-4 pt-3 bg-gradient-to-b from-black/80 via-black/40 to-transparent">
              <div className="flex items-center justify-between mb-2">
                <Link href="/games"><button className="w-8 h-8 rounded-full bg-black/50 border border-violet-500/30 flex items-center justify-center text-violet-300"><ArrowLeft size={15} /></button></Link>
                <div className="flex flex-col items-center gap-0.5">
                  <span className="font-display font-black text-2xl text-violet-300 leading-none drop-shadow-[0_0_10px_rgba(160,80,255,0.6)]">{scoreDisp}</span>
                  <span className="text-[9px] text-violet-400/50 font-display uppercase tracking-widest">GOAL {ticket?.target}</span>
                </div>
              ...
              <div className="flex flex-col gap-1 pb-3">
                <div className="flex items-center justify-between px-0.5"><span className="text-[9px] tracking-widest text-violet-300/60 font-display uppercase">Sorted</span><span className="text-[10px] font-mono font-bold text-white/70">{scoreDisp} / {ticket?.target}</span></div>
                <div className="w-full h-2 rounded-full bg-white/10 overflow-hidden border border-violet-500/20"><div className="h-full rounded-full bg-gradient-to-r from-violet-500 to-purple-400 shadow-[0_0_8px_rgba(150,80,255,0.5)] transition-[width] duration-300" style={{width:`${ticket?Math.min(100,(scoreDisp/ticket.target)*100):0}%`}}/></div>
                ...
                <div className="w-full h-1.5 rounded-full bg-white/8 overflow-hidden"><div ref={timerBarRef} className="h-full rounded-full bg-gradient-to-r from-violet-400 to-purple-400 transition-none" style={{width:"100%"}}/></div>
              </div>
            </div>
