      ...
      <AnimatePresence>{phase === "select" && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 z-40 flex flex-col px-6 pt-16 pb-6 overflow-y-auto" style={{ background: "rgba(10,0,21,0.97)" }}>
          <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="flex flex-col items-center text-center w-full max-w-[340px] mx-auto">
            ...
            {balance < 30 ? (<button onClick={refillBalance} data-testid="button-refill" className="mt-5 w-full py-3 rounded-2xl bg-gradient-to-r from-purple-600 to-pink-600 text-white font-mono font-bold text-sm tracking-widest shadow-[0_0_24px_rgba(155,89,182,0.5)] active:scale-95">🎁 GET 1,000 FREE CHIPS</button>) : (<button onClick={refillBalance} data-testid="button-refill" className="mt-3 text-[10px] text-white/25 hover:text-white/50 transition-colors underline underline-offset-2 font-mono">Low on chips? Get 1,000 free</button>)}
          </motion.div>
        </motion.div>
      )}</AnimatePresence>

      <AnimatePresence>{phase === "won" && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 z-40 flex items-center justify-center px-8" style={{ background: "rgba(10,0,21,0.93)" }}>
          <motion.div initial={{ scale: 0.85 }} animate={{ scale: 1 }} transition={{ type: "spring", bounce: 0.4 }} className="w-full max-w-[300px] flex flex-col items-center text-center">
            ...
            <span data-testid="text-result" className="text-xs tracking-[0.4em] font-display uppercase mb-2 text-purple-300">Breaker!</span>
            <div className="font-display font-black text-5xl text-purple-300 mb-1">+{ticket?.prize.toLocaleString()}</div>
            ...
            <div className="w-full grid grid-cols-2 gap-3 mb-7">
              <div className="bg-purple-700/8 border border-purple-700/30 rounded-2xl p-3 flex flex-col items-center"><div className="text-white/40 text-[10px] font-mono uppercase mb-1">Score</div><span data-testid="text-final-score" className="font-mono font-bold text-xl text-purple-300">{score}</span></div>
              <div className="bg-purple-700/8 border border-purple-700/30 rounded-2xl p-3 flex flex-col items-center"><div className="text-white/40 text-[10px] font-mono uppercase mb-1">Balance</div><span data-testid="text-balance-final" className="font-mono font-bold text-xl text-purple-300">{balance.toLocaleString()}</span></div>
            </div>
            <button onClick={() => setPhase("select")} data-testid="button-replay" className="w-full py-3.5 rounded-2xl bg-gradient-to-r from-purple-600 to-pink-500 text-white font-mono font-bold tracking-widest flex items-center justify-center gap-2 mb-3 active:scale-95"><RotateCcw size={18} />PLAY AGAIN</button>
            <Link href="/games"><button data-testid="button-exit" className="text-sm text-purple-300/40 hover:text-purple-300 font-mono">← Back to Arena</button></Link>
          </motion.div>
        </motion.div>
      )}</AnimatePresence>
      ...
      <AnimatePresence>{phase === "lost" && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 z-40 flex items-center justify-center px-8" style={{ background: "rgba(10,0,21,0.93)" }}>
          <motion.div initial={{ scale: 0.85 }} animate={{ scale: 1 }} transition={{ type: "spring", bounce: 0.35 }} className="w-full max-w-[300px] flex flex-col items-center text-center">
            <div className="text-5xl mb-3">💔</div>
            <span data-testid="text-result" className="text-xs tracking-[0.4em] font-display uppercase mb-2 text-red-400">Ball Lost!</span>
            ...
            <button onClick={() => setPhase("select")} data-testid="button-replay" className="w-full py-3.5 rounded-2xl bg-gradient-to-r from-purple-600 to-pink-500 text-white font-mono font-bold tracking-widest flex items-center justify-center gap-2 mb-3 active:scale-95"><RotateCcw size={18} />TRY AGAIN</button>
            <Link href="/games"><button data-testid="button-exit" className="text-sm text-purple-300/40 hover:text-purple-300 font-mono">← Back to Arena</button></Link>
          </motion.div>
        </motion.div>
      )}</AnimatePresence>
